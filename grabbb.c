 ...
 *
 * compilation:
 * linux/bsd: gcc -o grabbb grabbb.c -Wall
 * slowaris: cc -o grabbb grabbb.c -lsocket -lnsl
 */
...
#define	OPEN_MAX	250	/* hehe, don't mess with the mighty fdset's limits */
int		open_max = OPEN_MAX;
int		sock_free = OPEN_MAX;
sock_buck	sockets[OPEN_MAX];
...
void
sb_print (sock_buck *sb, char *buf)
{
	unsigned char	*foo;
	char		ip[64];
	...
	if (multiline == 0 && strlen (buf) > 0) {
		for (foo = buf ;
			((unsigned long)((unsigned long)foo -
			(unsigned long)buf) < strlen (buf)) &&
			*foo != '\r' && *foo != '\n' &&
			*foo != '\x00'; ++foo)
			;
		*foo = '\x00';
	...
	if (sb->printed == 0) {
		sb->printed = 1;
		stat_conn++;
		printf ("%s:%hu:%c%s", ip, sb->port[sb->portcount],
			(multiline == 0) ? ' ' : '\n', buf);
	} else {
		printf ("%s", buf);
	}
